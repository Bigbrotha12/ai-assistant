...
// "Bearer" and double-mask it).
const AUTHORIZATION_GENERIC = /Authorization\s*:\s*(?!Bearer\b)\S+/gi;
 ...
 * into message content). Masks `Authorization: <value>` headers, `Bearer
 * <token>` occurrences and `sk-...` API keys with `CREDENTIAL_REDACTION`
